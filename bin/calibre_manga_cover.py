...
parser = argparse.ArgumentParser(description='extract manga covers and install into calibre directory')
parser.add_argument('-e','--ext', type=str, default='cbz', help='filename extension', choices=['cbz'])
parser.add_argument('-m', '--manga-dir', type=str, default='.', help='directory for manga files')
parser.add_argument('-d', '--calibre-dir', type=str, default='~/Calibre Library', help='calibre library dir')
parser.add_argument('-a', '--calibre-author', type=str, default='KCC', help='author name')
...
calibre_dirs = [
    i for i in sorted(calibre_path.iterdir())
