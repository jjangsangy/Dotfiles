...
--                   Saves both video frame and audio spectrogram data to temp files.
...
--      - Performs FFT using an optimized Stockham Radix-4 Autosort algorithm.
...
--      - Audio FFT (FFI Path):
--          - Stockham Radix-4 Autosort: Avoids bit-reversal permutations.
--          - Mixed-Radix: Handles power-of-2 sizes (e.g. 2048 via Radix-2 + Radix-4).
--          - Cache Optimized: Uses planar (split-complex) data layout for efficient memory access.
--          - Twiddle Caching: Precomputed trigonometric tables to eliminate runtime sin/cos calls.
--      - Uses a sliding window search strategy centered on the saved timestamp (Video).
--      - Uses a linear scan strategy from the beginning of the file (Audio).
        ...
        if l == 1 then
            ...
            for k = 0, m - 1 do
                ...
                for j = 0, l - 1 do
                    ...
                    -- Data access is now unit-stride (j increases by 1)
                    local r0, im0 = x_re[i0], x_im[i0]
                    local r1, im1 = x_re[i1], x_im[i1]
                    local r2, im2 = x_re[i2], x_im[i2]
                    local r3, im3 = x_re[i3], x_im[i3]

                    -- Twiddle access is strided (stride m), but it's read-only
                    -- and the data arrays benefit most from packed loads/stores.
                    local w1r, w1i = t_re[j * m], t_im[j * m]
                    local w2r, w2i = t_re[j * 2 * m], t_im[j * 2 * m]
                    local w3r, w3i = t_re[j * 3 * m], t_im[j * 3 * m]
    ...
    if ffi_status then
        ...
        if options.audio_use_fftw == "yes" and ffi.os ~= "OSX" then
            ...
            if fftw_lib then
                ...
                local hann = ffi.new("float[?]", fft_size)
                ...
                local mag_buf = ffi.new("float[?]", fft_size / 2)
                ...
                for i = 0, num_frames - 1 do
                    ...
                    for k = 0, fft_size / 2 - 1 do
                        mag_buf[k] = math.sqrt(fft_out_c[k][0] ^ 2 + fft_out_c[k][1] ^ 2)
                    end
